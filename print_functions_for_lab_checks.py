...
# REVISED DATE: 23.11.2018 --> zero devision     <=(Date Revised - if any)                         
...
def check_creating_pet_image_labels(results_dic):
    ...
    if results_dic is None:
        ...
        for key in results_dic:
            ...
            if n < stop_point:
                # Why are we printing just the first character? <-- results_dic[key][0]
                # Are we expecting a list of labels in the dictionary?
...
def check_calculating_results(results_dic, results_stats_dic):
    ...
    if results_stats_dic is None:
        ...
        if n_pet_dog != 0:
            ...
            pct_corr_dog = 'not applicable'
            pct_corr_breed = 'not applicable'
        
        if n_pet_notd != 0:
            pct_corr_notdog = ( n_class_cnotd / n_pet_notd )*100
        else:
            pct_corr_notdog = 'not applicable'
        
    
        # prints calculated statistics
        print("\n ** Statistics from calculates_results_stats() function:")
        print("N Images: {:2d}  N Dog Images: {:2d}  N NotDog Images: {:2d} \nPct Corr dog: {} Pct Corr NOTdog: {}  Pct Corr Breed: {}".format(
